...
exports.initialize = function initializeDataSource(dataSource, callback) {
	dataSource.connector = new Blazestore(dataSource.settings);
	process.nextTick(() => {
		callback();
	});
};
...
class Blazestore {
	...
	/**
	 * Create new instance of model and verify if is Access Token type
	 * @param {String} model 
	 * @param {Object} data
	 */
	create(model, data, callback) {
		if (model == 'AccessToken') {
			this.db.collection(model).doc(data.id).set(data)
				.then(ref => callback(null, ref.id))
				.catch(err => callback(err));
		} else {
			this.db.collection(model).add(data)
				.then(ref => callback(null, ref.id))
				.catch(err => callback(err));
		}
	}
...
util.inherits(Blazestore, Connector);
exports.Blazestore = Blazestore;
